...
#This code below makes it a lot easier to program the game and the players actions
#from Steve Paget (Paget Teaches YT) github : https://github.com/stevepaget/pygame_functions
...
class newLabel(pygame.sprite.Sprite):
    def __init__(self, text, fontSize, font, fontColour, xpos, ypos, background, centreX, screenWidth):
        pygame.sprite.Sprite.__init__(self)
        self.text = text
        ...
        self.fontHeight = self.font.size(self.text)[1]
        self.fontWidth = self.font.size(self.text)[0]

        self.renderText()
        if centreX:
            xpos = (screenWidth / 2) - (self.fontWidth / 2)
        self.rect.topleft = [xpos, ypos]
    ...
    def renderText(self):
        ...
        self.image = pygame.Surface((maxWidth, (self.fontHeight) * len(textLines)), pygame.SRCALPHA, 32)
        #self.fontSize + 1
...
def screenSize(sizex, sizey, xpos=None, ypos=None, fullscreen=False, name = "Graphics Window"):
    global screen
    global background
...
def makeLabel(text, fontSize, xpos, ypos, fontColour='black', font='Arial', background="clear", centreX = False, screenWidth = 100):
    # make a text sprite
    thisText = newLabel(text, fontSize, font, fontColour, xpos, ypos, background, centreX, screenWidth)
    return thisText
