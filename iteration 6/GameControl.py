	...
	def menuLoop(self):
		...
		while True:
			...
			for event in pygame.event.get():
				if event.type == QUIT:
					...
					if event.key == K_UP:
						...
						position = position - 1
						if position < 0: # Ensure it can't go out of bounds
							position = 0
					elif event.key == K_DOWN:
						menu.currentPage.connectedPages[position].highlightOff()
						position = position + 1
						if position > numberOptions: # ensure it doesn't go below our list...
			...
			if play:
				...
				return # A neat way of moving into the game. may decide to send a list of config data instead
			...
			for event in pygame.event.get():
				if event.type == QUIT:
					...
					if event.key == K_BACKSPACE:
						...
						page.connectedPages[position].highlightOff()
						position = position - 1
						if position < 0: # ensure it doesnt go below the list...
							position = 0
